#code where user account log in is happening but dynamic tables are not showing in the database 
...
# Helper function to dynamically create the table
def create_user_account_table(accountname):
    table_name = f'user_accounts_{accountname}'

    class DynamicUserAccounts(db.Model):
        __tablename__ = table_name
        id = db.Column(db.Integer, primary_key=True)
        date = db.Column(db.DateTime, nullable=False)
        full_addr = db.Column(db.String(255))
        sensor_data = db.Column(db.String(255))
        vehicleno = db.Column(db.String(50))
        volume_liters = db.Column(db.Float)
    ...
    db.create_all()
    print(f"Model for table '{accountname}' created and stored.")
   




def create_user_account(accountname):
    # Create dynamic table for the user
    table_name = f'user_account_{accountname}'
    metadata = MetaData()
    ...
    user_account_table = Table(
        ...
        db.Column('sensor_data', db.Float),
        db.Column('vehicleno', db.String(50)),
        db.Column('volume_liters', db.Float)
    )

    metadata.create_all(db.engine)  # Create the table in the database

    print(f"Table {table_name} created for user {accountname}.")
...
def login():
    ...
    if request.method == 'POST':
        ...
        user = User.query.filter_by(email=email).first()  # Admin login
        user_account = UserAccount.query.filter_by(accountemail=email).first()  # Regular user login
        ...
        if user and user.check_password(password):
            ...
            return redirect(url_for('admin_dashboard', adminname=user.name))  # Redirect to admin dashboard

        elif user_account and user_account.check_password(password):
            # Regular user login
            session['email'] = user_account.accountemail
            session['is_admin'] = False
            session['accountname'] = user_account.accountname  # Store account name in session

            return redirect(url_for('user_dashboard', accountname=user_account.accountname))  # Redirect to user dashboard
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            try:
                ...
                api_logger.error("Database error for admin: %s, error: %s", adminname, str(db_error))
...
def create_user_account_route():
    if request.method == 'POST':
        ...
        new_user_account = UserAccount(
            accountname=accountname,
            accountemail=accountemail,
            accountpassword=generate_password_hash(accountpassword),
        ...
        create_user_account(accountname)

        return redirect(url_for('login'))

@app.route('/dashboard/user/<accountname>')
def user_dashboard(accountname):
    if 'email' in session:
        # Assuming accountemail is the correct column name
        user_account = UserAccount.query.filter_by(accountemail=session['email']).first()

        # Check if the user account exists
        if user_account is None:
            return redirect('/login')

        # Redirect if accountnames do not match
        if user_account.accountname != accountname:
            return redirect(url_for('user_dashboard', accountname=user_account.accountname))
        ...
        if not inspector.has_table(table_name):
            create_user_account_table(accountname)  # Ensure this function is defined properly
        ...
        try:
            ...
            query = db.session.query(table).order_by(table.c.date.desc())  # Ensure 'date' is a valid column
            pagination = query.paginate(page=page, per_page=per_page, error_out=False)
            sense_data = pagination.items  # Get the items for the current page

            # Pass 'pagination' to the template along with 'user_account' and 'sense_data'
            return render_template('user_dashboard.html', user=user_account, sense_data=sense_data, pagination=pagination)

        except NoSuchTableError:
            return "Error: Table does not exist.", 500
